...
class baselineDataset(Dataset):
    ...
    def __getitem__(self, index):
        with h5py.File(self.h5_path, "r") as f:
            ...
            eigmode = eigvals[None,:] * eigvecs # (N, N), complex 
            eigmode = torch.stack((eigmode.real, eigmode.imag), dim=-1) # (N, N, 2), float32
            eigmode = eigmode.permute(1, 0, 2)   
            eigmode = eigmode.reshape(eigmode.shape[0], -1) # (N, 2N)
