...
from .datasets.precomputed_dataset import precomputedDataset
...
# --- Utility function to run one full pass over the training set --- #
def train_epoch(model, loader, optimizer, device, gradient_clip_max_norm):
    model.train()
    total_loss = 0.0
    total_loss_loc = 0.0
    total_loss_str = 0.0
    ...
    for data in loader:
        ...
        total_loss_str += loss_str.item()

        pred_locs.append(pred_loc.detach().cpu())

    avg_loss = total_loss / len(loader)
    avg_loss_loc = total_loss_loc / len(loader)
    avg_loss_str = total_loss_str / len(loader)

    pred_std = torch.cat(pred_locs, dim=0).std(dim=0).mean().item()

    return avg_loss, avg_loss_loc, avg_loss_str, pred_std

# --- Utility function to train model from a config ---
def run_training(config, checkpoint_path=None):
    ...
    train_loader = PyGDataLoader(train_ds, batch_size=config["train_batch_size"], shuffle=True,  num_workers=4, pin_memory=pin_memory)
    val_loader   = PyGDataLoader(val_ds,   batch_size=config["val_batch_size"],   shuffle=False, num_workers=4, pin_memory=pin_memory)
    ...
    for epoch in range(1, config["epochs"] + 1):
        ...
        # --- Training --- #
        train_loss, train_loss_loc, train_loss_str, pred_std = train_epoch(model, train_loader, optimizer, device, config["gradient_clip_max_norm"])
